...
const NavLink = ({ to, children }) => (
    <Link to={to} className="relative text-gray-600 hover:text-primary transition-colors py-2 group font-medium">
        {children}
...
const Navbar = () => {
    ...
    return (
        <motion.nav
            ...
            className={`fixed w-full z-50 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${scrolled ? "bg-white/80 backdrop-blur-md border-b border-gray-100 py-0" : "bg-transparent border-transparent py-4"
                }`}
        >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className={`flex justify-between items-center transition-all duration-500 ${scrolled ? "h-16" : "h-20"}`}>
                    <div className="flex-shrink-0 flex items-center">
                        <span className="text-2xl font-bold tracking-tighter text-gray-900">Agencia<span className="text-primary">.</span></span>
                    </div>
                        ...
                        <div className="px-4 pt-2 pb-4 space-y-1">
                            {navLinks.map((link, index) => (
                                <motion.div key={index} variants={{ open: { opacity: 1, x: 0 }, closed: { opacity: 0, x: -20 } }}>
                                    <Link onClick={() => setIsOpen(false)} to={link.path} className="block px-3 py-3 text-gray-600 hover:text-primary font-medium rounded-lg hover:bg-gray-50">{link.title}</Link>
                                </motion.div>
                            ))}
                        </div>
                    </motion.div>
